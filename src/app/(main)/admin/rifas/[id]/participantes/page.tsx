...
export default function AdminRaffleParticipantsPage() {
  ...
  const enrichPurchasesWithUserData = async (
    purchases: NumberPurchase[]
  ): Promise<Purchase[]> => {
    ...
    for (const purchase of purchases) {
      try {
        ...
        const enrichedPurchase: Purchase = {
          ...
          userName:
            userData?.displayName || userData?.name || 'Usuario desconocido',
        ...
        console.error(
          'Error fetching user data for purchase:',
          purchase.id,
          error
        );
  ...
  const filteredParticipants = participants
    .filter(participant => {
      ...
      if (
        ...
        participant.paymentMethod !== filters.paymentMethod
      ) {
        return false;
      }
  ...
  const totalNumbers = participants.reduce(
  ...
  const handleSendNotifications = async () => {
    try {
      ...
      alert(
        `Notificaciones enviadas a ${filteredParticipants.length} participantes`
      );
    } catch (error) {
    ...
    return (
      <div className="p-6 text-center">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Verifica que el ID sea correcto y que la rifa exista en la base de
          datos.
        </p>
